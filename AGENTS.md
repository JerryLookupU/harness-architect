...
- 每次新回合开始时，尤其是压缩上下文后，先把 `docs/phase1-validation-loop.md` 当作第一阶段主声明重新读取，再决定动作。
- 进入动作前，先用一句话确认当前角色关系：`Klein-Harness` 是本体项目，当前验证仓库是目标项目。
- 第一阶段默认闭环不可跳过：修改本体项目 -> 安装到目标项目 -> 向目标项目提交真实需求 -> 观察目标项目是否通过 harness 自己完成需求。
- 如果目标项目没有完成需求，默认解释为 `harness` 能力缺口；先回到本体项目修模板、脚本、控制面、spec 或安装链，不直接手改目标业务代码。
- 每次真实运行都要先回答“当前 blocker 暴露的是提示词缺口还是系统缺口”，然后才允许继续迭代。
- 如果当前计划没有显式包含这条本体/目标循环，先收敛回该循环，再继续执行。
...
- 第一阶段的严格角色关系：
- `Klein-Harness` 是本体项目；当前仓库只是它的本体仓库目录，只允许在这里修改 harness 模板、脚本、控制面和安装链。
- 目标业务仓库只作为需求提交与验收场，不是本轮主要修改对象。
- 第一阶段的默认闭环是：修改本体项目 -> 安装到目标项目 -> 向目标项目提交真实需求 -> 观察目标项目是否通过 harness 流程完成需求；若未完成，则回到本体项目继续迭代。
...
- 对目标仓库的真实运行，默认是给 `Klein-Harness` 做验收和采样；采到的问题需要回灌到本体仓库，沉淀成可复用机制，而不是只修当前仓库这一次。
- 当目标是通过 harness 驱动业务仓库时，默认角色是 operator / harness maintainer，不直接修改业务仓库代码。
- 对目标项目，优先通过 `harness-submit <root> --goal "<需求>"` 提交需求，再观察 `.harness` 执行链。
- 发现任务执行异常时，先判断是提示词未遵循规范，还是 harness 系统能力缺口；不要先入为主地改业务代码。
- 只有在确认是 harness 问题后，才允许修改当前本体仓库内的模板、脚本、安装链，然后重装到目标项目验证。
